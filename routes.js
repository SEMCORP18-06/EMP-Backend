...
router.post('/enquiries', authenticateToken, requireActiveRole, async (req, res) => {
  try {
    ...
    const savedEnquiry = await Enquiry.create({
      ...
      majorEquipments: (majorEquipments || '').trim() || '-',
      enquirySource: (enquirySource || '').trim() || '-',
      fpr: cleanSalutations(fpr) || '-',
      mailId: (mailId || '').trim() || '-',
      contactCountryCode: normalizedCC || '+91',
...
const generateExcelBuffer = (enquiries) => {
  ...
  XLSX.utils.book_append_sheet(wb, ws, "Weekly Enquiries");
  return XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });
};

// Core logic to trigger and send the weekly email report containing enquiries entered in the last week
export const sendWeeklyReportMail = async () => {
  ...
  if (!enquiries || enquiries.length === 0) {
    ...
    return { message: 'No enquiries in the last week. Report not sent.' };
  }
  ...
  if (!admins || admins.length === 0) {
    ...
    return { message: 'No Admin users found. Report not sent.' };
  }

  const adminEmails = admins.map(a => a.username).filter(Boolean);
  if (adminEmails.length === 0) {
    console.log('[Weekly Report] No Admin email addresses found. Skipping email.');
    return { message: 'No Admin email addresses found. Report not sent.' };
  }

  // Generate Excel buffer
  const excelBuffer = generateExcelBuffer(enquiries);
...
router.get('/cron/weekly-report', async (req, res) => {
  ...
  try {
    const result = await sendWeeklyReportMail();
